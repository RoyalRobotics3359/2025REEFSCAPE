...
public class OperatorConsole {
    ...
    public class LogitechController {
        ...
        // Uses a lambda function to return boolean value if trigger has been pressed
        public Trigger leftTrigger() {
            return new Trigger(() -> (controller.getRawAxis(2) >= TRIGGER_DEADBAND));
        }

        public Trigger rightTrigger() {
            return new Trigger(() -> (controller.getRawAxis(3) >= TRIGGER_DEADBAND));
        }
        ...
        public double getLeftStickY() {
            // Multiply by -1 to compensate for reversal of left stick data when read
            double leftY = -1.0 * controller.getRawAxis(1);
            if (leftY <= -1.0 * STICK_DEADBAND || leftY >= STICK_DEADBAND) {
                return leftY;
            }
            return 0.0;
        ...
        public double getLeftStickX() {
            if (controller.getRawAxis(0) <= -STICK_DEADBAND || controller.getRawAxis(0) >= STICK_DEADBAND) {
        ...
        // Right stick data read with applied deadband range
        public double getRightStickY() {
            if (controller.getRawAxis(5) <= -STICK_DEADBAND || controller.getRawAxis(5) >= STICK_DEADBAND) {
        ...
        public double getRightStickX() {
            if (controller.getRawAxis(4) <= -STICK_DEADBAND || controller.getRawAxis(4) >= STICK_DEADBAND) {
    ...
    private LogitechController gameController;

    public OperatorConsole() {
        driveController = new Controller(0);
        gameController = new LogitechController(1);
    }

    public Controller getDriveController() { return driveController; }

    public LogitechController getGameController() { return gameController; }
